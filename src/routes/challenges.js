...
router.get('/new/:opponentId', requireAuth, requirePaid, (req, res) => {
  ...
  const opponent = db.prepare('SELECT id, name, sport, skill_level, rank FROM players WHERE id = ?').get(opponentId);
  if (!opponent || opponent.id === req.session.userId) {
    return res.status(404).send('Opponent not found');
  }

  const challenger = db.prepare('SELECT id, sport, skill_level, rank FROM players WHERE id = ?').get(req.session.userId);
  if (!challenger) return req.session.destroy(() => res.redirect('/login'));

  if (opponent.sport !== challenger.sport) {
    return res.status(400).send('You can only challenge players in the same sport.');
  }
  if (opponent.skill_level !== challenger.skill_level) {
    return res.status(400).send('You can only challenge players in the same skill level.');
  }
...
router.post('/report', requireAuth, requirePaid, (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    recalcRanks(db, challenge.season_id, sport, skill_level, challenge.challenger_id, challenge.opponent_id, winnerId);
  });
